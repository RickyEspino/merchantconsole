...
export async function GET(req: Request) {
  ...
  // 1) fetch token, ensure open & unexpired
  ...
  if (error || !token) return NextResponse.json({ error: 'Invalid token' }, { status: 400 });
  ...
  // 2) credit points to DEMO user’s wallet
  ...
  if (!wallet) return html('No wallet', 'Demo wallet not found.');

  // Optional: which tenant to attribute to? For now, none/NULL or a default.
  const { error: insErr } = await admin.from('points_ledger').insert({
    wallet_id: wallet.id,
    tenant_id: null,            // or set a default tenant id if you want attribution
    event_type: 'issue',
    delta: token.points,
    reason: 'QR earn',
  });
  ...
  // 4) confetti success page (auto close/redirect)
