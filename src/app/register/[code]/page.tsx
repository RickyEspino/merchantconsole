...
import { requireMerchant, resolveUserClaimBaseForMerchant } from "@/lib/merchant";
...
export default async function QRPage({ params }: { params: { code: string } }) {
  ...
  // 🔑 Build the claim URL on the correct USER app subdomain for this merchant's tenant
  const userBase = await resolveUserClaimBaseForMerchant(merchant);
  const claimUrl = new URL(`/claim?code=${encodeURIComponent(token.code)}`, userBase).toString();
